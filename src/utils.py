def clean_query_intent(query: str) -> str:
    """
    Remove Arabic intent/stop phrases and extract main keyword/intent.
    Examples:
        "عاوز اتعلم بايثون" -> "بايثون"
        "تعلم SQL" -> "SQL"
        "عايز Data Analysis" -> "Data Analysis"
    """
    # Remove intent/stopwords at the start
    q = query.strip()
    # Remove all stopwords at the start
    tokens = q.split()
...
def remove_arabic_letters(text: str) -> str:
    """
    Remove stray Arabic letters from an English token sequence.
    Example: "اlearning python" -> "learning python"
    """
    # Remove Arabic unicode chars
