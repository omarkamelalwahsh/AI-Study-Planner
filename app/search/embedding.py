...
def has_subject(query: str) -> bool:
    ...
    # Remove learning keywords to see if there is actual subject matter
    for kw in LEARNING_KEYWORDS:
        # distinct word replacement to avoid partial matches inside words if needed
        # but normalize_ar lowercases everything.
        # simple replace for now, or regex word boundary is better.
        norm_q = re.sub(rf"\b{re.escape(kw)}\b", "", norm_q)
        
    # Also remove common opinion words if you want to be strict, 
    # but `is_generic_opinion_query` handles the pattern check.
    # `has_subject` is called BY `is_generic_opinion_query`.
    # So we just want to know if there's anything substantial left.
    
    clean = re.sub(r"\s+", " ", norm_q).strip()
    return len(clean) >= 2
